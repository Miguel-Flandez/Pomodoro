...
export default function HabitTracker({ showAddModal,  setShowAddModal, habits, setHabits,
    ...
    return(
        ...
        <div className="flex gap-[1vw] w-[90vw] justify-center m-auto">

            <div id="daily" className="flex flex-col gap-[1vw] w-[25%] h-[80vh] bg bg-[#1211112f] rounded-xl p-[1rem]">
                    ...
                    {habits.map((habit,index)=>{
                        ...
                        date.setDate(todate.getDate())
                           ...
                           return habit.schedule.find(e=>e===todate.toLocaleString('default',{weekday:'short'})) && (
                            ...
                            <button key={index} className="flex justify-between items-center min-h-[20%] text-[1.5rem] relative rounded-xl overflow-hidden px-[1rem]"
            ...
            {/* weekly habits */}
            <div id='weekly' className='w-[70%] h-[80vh] bg-[#1211112f] p-[1rem] rounded-xl overflow-auto'>
                <div id="grid" className='grid grid-cols-[3fr_repeat(7,1fr)_3fr] gap-[2vw] items-center grid-template-rows: auto 1fr;'>
                    <div id="weekly-header" className='text-[1rem] font-bold flex flex-col items-center'>
                        ...
                        <React.Fragment key={index}>
                            ...
                            return(
                                <React.Fragment key={id}>
                                    {habit.schedule?.find(n=>n===day) && <button className={` border-2 overflow-hidden rounded-[50%] w-[2rem] h-[2rem]`}
                                    onClick={()=>{checkHabit(index, date.getDate());popSound.play()}}>
